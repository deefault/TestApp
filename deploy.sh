...
dotnet restore "$DEPLOYMENT_SOURCE/WebApplication3.sln"
exitWithMessageOnError "dotnet restore failed"

# 2. Build and publish
dotnet publish "$DEPLOYMENT_SOURCE/WebApplication3/WebApplication3.csproj" --output "$DEPLOYMENT_TEMP" --configuration Release
...
if [ -e "$DEPLOYMENT_TARGET/bower.json" ]; then
  ...
  #TODO: fix
  [ ! -d "$DEPLOYMENT_TARGET/wwwroot/lib" ] && mkdir -p "$DEPLOYMENT_TARGET/wwwroot/lib"
  mv ./bower_components/* ./wwwroot/lib/ 
